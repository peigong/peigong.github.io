...
郭守敬（1231–1316）在测量领域（含天文测量、大地/地形测量、时间测量）的贡献，是元代科技发展的最高代表之一。他不只是"改进了几件仪器"，而是建立了一套较完整的观测—计量—推算体系。主要可归纳为以下几方面：
...
- 简仪（简化浑仪）
   - 将传统浑仪重叠的环组拆成独立测定赤经–赤纬（赤道式）和地平坐标的两套环组，减少遮掩、便于对准。
   - 加装窥衡+十字线照准装置（早期类似现代望远镜十字丝），提高星体、太阳的照准精度。
   - 可同时测赤道坐标和地平高度，是当时世界领先的赤道式天文仪器。
- 高表（四丈圭表）
   - 将传统八尺之表加高至四丈（约12米），并在表顶加横梁，投影落在长圭面上。
   - 圭面设景符（带小孔的反光片），利用针孔成像使日影边缘清晰，极大提高冬至、夏至晷影长度的测量精度——这是修订回归年长度（≈365.2425日）的关键。
- 其他辅助仪器：仰仪（仰天锅形，可在日食时直接读出时刻与食分）、正方案（可测太阳方位角，用于定方向、测地理纬度）等。

2. 组织大规模全国天文大地测量（《授时历》基础）

- 在全国27个观测点（最北至北海/西伯利亚以南，最南至南海诸岛附近）同步测：
   - 北极出地高度（地理纬度φ）
   - 冬/夏至及春分秋分晷影长
   - 昼夜长短
- 这是中国古代规模最大、最系统的天文大地测量，验证了"寸差千里"经验规律的不准确，并为《授时历》提供了真实数据基础。

#### 二、大地测量与地形水准测量（水利工程应用）

3. 改进"水平"为野外水准测量仪

如前所述：

- 把连通器式"水平"改为带精细刻度水槽＋可调支座＋望筒照准＋前后视标尺的系统；
- 用于通惠河、大都供水、运河引水等工程中，沿线路实测地面高差与坡度——是中国古代少有的系统化水准测量实践。

4. 地形勘察与线路测量

- 主持踏勘华北平原至大都周边山川、水系；
- 结合水准测量与距离丈量，选定引水路线与坝址，体现早期的工程测量思想（高程控制＋平面走向）。

#### 三、时间测量与计量标准

5. 精密测定回归年长度 → 《授时历》

- 用高表+景符测得极精确的冬/夏至晷影，算出：1回归年 = 365.2425日
- 与现行公历（格里高利历）数值相同，但早约300年。
- 《授时历》废除积年日法，采用实测数据推算，是中国古代历法由"象数推演"转向实测—拟合—推算的典范。

6. 改进计时与校时装置

- 参与设计/改进机械漏刻（多级恒定流量漏壶＋补偿机构），提高守时精度；
- 用天文观测（星过中天、日影）定期校准漏刻，形成观—计—校闭环。

#### 四、方法与理念上的贡献（常被忽视）

层面 | 郭守敬的突破
---------------------------
观测思想 | 强调"先测而后算"，以大量实地数据否定旧有经验公式
仪器设计 | 简化结构、增大尺度、加照准/读数辅助（十字线、景符）——追求最小人为误差
测量体系 | 把天文坐标测量、地理纬度测定、地形水准测量、时间计量整合进同一国家工程（《授时历》+水利）
工程应用 | 将精密测量结果直接用于运河坡降、城市供水等大型土木工程

#### 📌 一句话概括

郭守敬在中国测量史上的真正贡献是：创制并运用高精度天文与水准测量仪器，组织全国范围的天文大地测量，以实测数据修订历法与指导大型水利工程，初步形成了古代中国较为完整的天文—大地—地形测量体系。

### 地平高度测量

#### 一、郭守敬在地平高度（含北极出地高度）测量方面的贡献

古代所说的"地平高度测量"在郭守敬工作中主要指两方面——天体地平高度（北极出地高度＝地理纬度）和地形水准高差（地面"地平"高低）：

##### 🔭 天体地平高度——"北极出地高度"的测定（四海测验）

- 创制/改进仪器：用自制的简仪（赤道式+地平式分离环组，带窥衡照准）测北天极地平高度；用四丈高表+景符精确测冬夏至影长，反推北极出地高度（φ）；用仰仪可直接读太阳地平高度与方位。
- 组织"四海测验"：至元十六年（1279）在全国设27个观测站（北至西伯利亚叶尼塞河，南至南海），统一使用简仪、高表、仰仪，首次大规模实测各地北极出地高度（即当地地理纬度），平均误差<0.35°，部分站点零误差，用于编制《授时历》。
- 认识子午线一度对应南北距离：通过不同测站北极高度差与实地距离之比，隐约触及"北极出地高度差与纬度、子午线弧长的关系"，在当时极具先进性。

##### 📏 地形"地平"高差——水准测量

- 用改进的水准水平仪（带刻度水槽＋望筒＋前后视标尺），沿通惠河、黄淮平原实测地面高差与坡降，判断可通航与否。
- 以海平面为统一基准比较大都（北京）与汴梁（开封）地形高下之差，这是世界上最早以平均海面为高程基准的思想雏形（见下文）。

#### 二、当时相当于"海拔"的概念古称

郭守敬当时没有"海拔"这个现代汉语词，文献原文记载是：

    "以海平面较京师至汴梁地形高下之差"—— 《元朝名臣事略》

关键古称要点：

- 基准面：称"海平面"（或"海面"），即以平均海水面为起算基准——这在世界上是最早提出用海平面作统一高程基准的记录，比高斯1828年提出平均海平面基准早约560年。
- 高差描述：称"高下之差"或"地形高下"（裴秀"制图六体"中已有"高下"指相对高程）。
- 现代学者认为郭守敬此举是世界上最早提出"海拔（标高/高程以海平面为基准）"概念的雏形，但在元代文献中并无"海拔"二字，只记作"以海平面较……高下之差"。

#### 一句话总结：

郭守敬用简仪、高表、仰仪在全国实测北极出地高度（地平高度→纬度），并用改进水准仪以海平面为基准比较远近两地地形高差——后者文献记为"以海平面较……高下之差"，是古代对"海拔高程系统"的最早实践，当时称基准为海平面，高差为高下（之差）。

### 改进水准测量仪

郭守敬（元代天文学家、水利学家）在测量仪器方面的重大贡献之一，是对早期水平仪（古代称"水平"）进行了改进，使其更适于精密的水准测量和地形勘测，尤其用于大运河引水、大都（今北京）供水等大型工程。

简单说他的主要改革可以归纳为以下几点：

#### 🔧 一、原始"水平"的局限

汉代以来已有"水平"（利用连通器原理的水槽加刻度），但存在：
...
#### ⚙️ 二、郭守敬对"水平"的改进（制成更完善的水准测量仪）

据《元史·天文志》《河渠志》及后世考据，他的改良主要包括：

1. 加大并规范连通水槽（水平槽）
   - 制作更大的长方形水槽，内盛水，利用连通器液面自然水平的原理。
   - 槽边刻有更精细的分划刻度，可读取微小倾斜。
2. 增设瞄准与定位装置
   - 在水平槽上配合立表（垂直标杆）与望筒（或类似照准设备），用于照准远处标尺或目标点，使高差测量更系统——这是向现代水准仪"视线水平 + 前后视标尺"思路的过渡。
3. 增加三脚架或可调支座
   - 使仪器能安平在地面上，通过微调让水面与刻线重合，确认视线水平。
4. 配套使用"景表""丈尺"等标尺
   - 在前方立带有分划的木尺（标尺），分别读前后视数，计算地面高差——类似今天水准测量的前后视法。
5. 用于大规模地形勘测
   - 郭守敬用这套改进的水平仪器沿大运河、通惠河线路实测地形坡度，确定引水高程，是古代少见的系统性水准测量应用实例。

#### 📌 总结一句话

郭守敬把古代简单的连通器式"水平"，改造成带精细刻度水槽、可调平支座、配瞄准装置和标尺进行前后视读数的水准测量器具，实质上已具备早期水准仪（Leveling Instrument）的功能，并成功用于元代大型水利工程地形测量。
